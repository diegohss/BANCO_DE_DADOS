use havik
go
select ent.nome_usuario resp_entrega, 
	   count(distinct proj.id_empresa) qtd_clientes
...
where datediff(month,proj.dt_criacao,DATEADD(day,-2,getdate()))<=4
and emp.id not in (5,6)
--proj.dt_fim is null

group by ent.nome_usuario